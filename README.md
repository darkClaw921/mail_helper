...
Скрипт [`setup.sh`](./setup.sh) делает всё за один запуск:
...
2. Ставит npm-зависимости в `webapp/` (если нет `node_modules`).
3. Копирует `webapp/.env.example` → `webapp/.env` (если отсутствует).
4. Генерирует `MASTER_KEY` (64 hex) в `.env`, если пустой.
5. Инициализирует SQLite (`npm run init-db`) идемпотентно — применяет новые поля схемы при повторных запусках.
6. Проверяет занятость `PORT`, автоматически переключает на свободный (`3100/3200/3300/4000/4100/5000/8000/8080/8888`) и пишет в `.env`.
7. Выводит **API key** (через `auth.ensureApiKey`), URL backend/WebSocket/Web UI и путь для `Load unpacked`.
8. Запускает `npm run dev` (backend + Vite).

Требования: `bash`, `node>=20`, `npm`. Работает на macOS (BSD sed) и Linux (GNU sed). Остановка — `Ctrl+C`.

Если нужно ручное управление шагами — смотрите пункты 1–5 ниже.
